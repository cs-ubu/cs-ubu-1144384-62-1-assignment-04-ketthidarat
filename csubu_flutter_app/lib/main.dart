...
class _AppHomePageState extends State<AppHomePage> {
  ...
  _getStudents() async {
    var url = 'http://cs.sci.ubu.ac.th:7512/topic-1/Thidarat_60114440143/_search?from=${_page*10}&size=10';
    ...
    if (response.statusCode == 200) {
      ...
      result.forEach((item) {
        if (item.containsKey('_source')) {
          ...
          if (source.containsKey('brand') && source.containsKey('color')) {
  ...
  Widget studentWidgets(BuildContext context) {
    return ListView.separated(
        ...
        itemBuilder: (context, i) {
          ...
          var sum = 0;
          student['color'].runes.forEach((c) { sum += c; });
          return ListTile(
            title: Row(
                  children: <Widget>[
